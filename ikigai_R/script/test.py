...
range_front = [0.0] * 10  # Initialize with zeros
range_right = []
range_left = []
min_front = 0
i_front = 0
...
while not rospy.is_shutdown():
    ...
    # 2. Start following the left wall.
    # 3. If too close to the left wall, reverse a bit to get away
    # 4. Otherwise, follow the wall by zig-zagging along the wall
    # 5. If the front is close to a wall, turn until clear
    while (near_wall == 0 and not rospy.is_shutdown()):  # 1
        ...
        if (min_front > 0.2 and min_right > 0.2 and min_left > 0.2):
            command.angular.z = -0.1  # if nothing is near, go forward
            command.linear.x = 0.15
            print("C")
        elif (min_left < 0.2):  # if wall on the left, start tracking
            near_wall = 1
            print("A")
        else:
            command.angular.z = -0.25  # if not on the left, turn right
            command.linear.x = 0.0
